...
namespace chfs
{
  ...
  TEST_F(MetadataServerTest, CheckInvariant4)
  {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      if(unlink_res==false){
        auto dir_content=meta_srv->readdir(dir_id_1);
    ...
    for (int i = 0; i < 100; i++) {
      ...
      if(unlink_res==false){
        auto dir_content=meta_srv->readdir(dir_id_2);
        for(auto &item:dir_content){
          std::cout<<item.first<<" \t"<<item.second<<std::endl;
        }
        std::cout<<file_id<<std::endl;
    ...
    if(dir_content_1.size()>1){
      for(auto &item:dir_content_1){
        std::cout<<item.first<<" \t"<<item.second<<std::endl;
      }
    }
    ...
    if(dir_content_2.size()>1){
      for(auto &item:dir_content_2){
        std::cout<<item.first<<" \t"<<item.second<<std::endl;
      }
    }
