...
namespace chfs {
    ...
    // {transaction id, metadata server block id, operation saving block id}
    using LogEntry = std::tuple<txn_id_t, block_id_t, block_id_t>;
    ...
    public:
        std::shared_ptr<std::vector<std::shared_ptr<BlockOperation>>> log_entry_;
        usize log_blocks;

        auto append_to_entry(block_id_t block_id, std::vector<u8> buffer) -> ChfsNullResult {
            if (this->log_entry_ == nullptr) {
                return this->write_block(block_id, buffer.data());
            } else {
//                std::cout << "add 1 log." << std::endl;
                std::shared_ptr<BlockOperation> op = std::make_shared<BlockOperation>(block_id, buffer);
                this->log_entry_->emplace_back(op);
                return KNullOk;
            }
        }
    ...
    public:
        ...
        auto flush_cur_block() -> ChfsNullResult {
            ...
            return this->bm->append_to_entry(target_block_id, this->buffer);
