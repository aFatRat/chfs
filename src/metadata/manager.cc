...
#define LOGIC_2_RAW(i) (i - 1)
        ...
        -> ChfsResult<inode_id_t>
    {
             ...
             iter.next(bm->block_size()).unwrap(), count++)
        {
            ...
            if (free_idx)
            {
                ...
                inode_id_t inode_id = RAW_2_LOGIC(free_idx.value());

                // 2. Setup the inode table.
                std::vector<u8> buffer(bm->block_size());
                // inode.flush_to_buffer(buffer.data());
                *(Inode *)(buffer.data()) = inode;
                // if (bm->write_block(bid, buffer.data()).is_err())
                // {
                //     return ChfsResult<inode_id_t>(ErrorType::INVALID);
                // }
                // auto op = std::make_shared<BlockOperation>(new BlockOperation(bid, buffer));
                // bm->log_entry_->emplace_back(op);
                bm->append_to_entry(bid, buffer);

                if (set_table(LOGIC_2_RAW(inode_id), bid).is_err())
                {
                    return ChfsResult<inode_id_t>(ErrorType::INVALID);
                }
                //                printf("Allocate inode. BID:%lu\tINODE_ID:%u\n",bid,free_idx.value());
    ...
    auto InodeManager::set_table(inode_id_t idx, block_id_t bid) -> ChfsNullResult
    {
        ...
        auto inode_block_id = idx / inode_per_block + 1;
        auto inode_block_idx = idx % inode_per_block;
        //        printf("SET:%lu,%lu\n",inode_block_id,inode_block_idx);

        std::vector<u8> buffer(bm->block_size());
        if (bm->read_block(inode_block_id, buffer.data()).is_err())
        {
        ...
        {
            buffer[i + inode_block_idx * sizeof(block_id_t)] = (char)((bid >> (i * sizeof(block_id_t))) & 0xFF);
        }

        // if (bm->write_block(inode_block_id, buffer.data()).is_err())
        // {
        //     return ChfsNullResult(ErrorType::INVALID);
        // }
        // auto op = std::make_shared<BlockOperation>(new BlockOperation(inode_block_id, buffer));
        // bm->log_entry_->emplace_back(op);
        bm->append_to_entry(inode_block_id, buffer);
        //        UNIMPLEMENTED();
    ...
    auto InodeManager::get(inode_id_t id) -> ChfsResult<block_id_t>
    {
        ...
        inode_id_t inode_id = LOGIC_2_RAW(id);

        auto inode_block_id = inode_id / inode_per_block + 1;
        auto inode_block_idx = inode_id % inode_per_block;
        //        printf("GET:%lu,%lu\n",inode_block_id,inode_block_idx);
        ...
        {
            //            buffer[i + inode_block_idx * sizeof(block_id_t)] = (char) ((bid >> (i * sizeof(block_id_t))) & 0xFF);
            block_id |= ((unsigned long long)buffer[i + inode_block_idx * sizeof(block_id_t)] << (i * sizeof(block_id_t)));
        }
        res_block_id = block_id;
...
//        printf("Read inode. Inode id:%lu, block id:%lu\n",id,block_id.unwrap());
    ...
    auto InodeManager::free_inode(inode_id_t id) -> ChfsNullResult
    {
        ...
        auto inode_id = LOGIC_2_RAW(id);
        if (set_table(inode_id, 0).is_err())
        {
            return ChfsNullResult(ErrorType::INVALID);
        }
        auto bitmap_id = inode_id / (bm->block_size() * KBitsPerByte) + (1 + n_table_blocks);
        auto bitmap_idx = inode_id % (bm->block_size() * KBitsPerByte);

        std::vector<u8> buffer(bm->block_size());
        if (bm->read_block(bitmap_id, buffer.data()).is_err())
        {
            return ChfsNullResult(ErrorType::INVALID);
        }
        Bitmap bitmap(buffer.data(), bm->block_size());
        bitmap.clear(bitmap_idx);
        // if (bm->write_block(bitmap_id, buffer.data()).is_err())
        // {
        //     return ChfsNullResult(ErrorType::INVALID);
        // }
        // auto op = std::make_shared<BlockOperation>(new BlockOperation(bitmap_id, buffer));
        // bm->log_entry_->emplace_back(op);
        bm->append_to_entry(bitmap_id, buffer);
