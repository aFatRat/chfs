...
namespace chfs
{
  ...
  inline auto meta_lock_num(inode_id_t inodeId) -> u8 { return inodeId % MetaMtxNum; }
  ...
  public:
    ...
    auto meta_lock_all()
    {
      for (int i = 0; i < MetaMtxNum; i++)
      {
        meta_mtx[i].lock();
      }
    }

    auto meta_unlock_all()
    {
    ...
    auto data_lock_all()
    {
      for (int i = 0; i < DataMtxNum; i++)
      {
        data_mtx[i].lock();
      }
    }

    auto data_unlock_all()
    {
      for (int i = DataMtxNum - 1; i >= 0; i--)
      {
        data_mtx[i].unlock();
      }
    }
