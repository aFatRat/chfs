...
namespace chfs
{
    ...
    auto FileOperation::alloc_inode(InodeType type) -> ChfsResult<inode_id_t>
    {
        ...
        //        auto free_inode_cnt = inode_manager_->free_inode_cnt().unwrap();
        //        auto allocator = BlockAllocator(block_manager_, inode_manager_->get_reserved_blocks());
        ...
        //        UNIMPLEMENTED();
        //        printf("Allocate INODE ID:%lu,BLOCK ID:%lu\n",inode_res.unwrap(),free_block);
        return inode_res;
    }
        ...
        -> ChfsNullResult
    {
        ...
        bool is_indirect_block_in_memory = false;
        //        printf("vec space:%zu\n",indirect_block.capacity());
        ...
        if (new_block_num > old_block_num)
        {
            ...
            for (usize idx = old_block_num; idx < new_block_num; ++idx)
            {
                ...
                {
                    auto indirect_res = inode_p->get_or_insert_indirect_block(block_allocator_);
                    if (indirect_res.is_err())
                    {
                        return ChfsNullResult(ErrorType::INVALID);
                    }
                    block_id_t indirect_block_id = indirect_res.unwrap();
                    //                    printf("Indirect block id:%lu\n", indirect_block_id);
                    if (!is_indirect_block_in_memory)
                    {
                        //                        printf("read indirect block,block id is:%lu\n",indirect_block_id);
                        if (block_manager_->read_block(indirect_block_id, indirect_block.data()).is_err())
                        {
                            return ChfsNullResult(ErrorType::INVALID);
                        }
                        is_indirect_block_in_memory = true;
                    }

                    auto block_idx = idx - inlined_blocks_num;
                    //                    printf("block id:%lu\n",block_id);
                    for (int i = 0; i < sizeof(block_id_t); i++)
                    {
                        //                        printf("set indirect block[%lu]:%d\n", i + block_idx * sizeof(block_id_t), (int) (
                        //                                (block_id >> (i * sizeof(block_id_t))) & 0xFF));
                        indirect_block[i + block_idx * sizeof(block_id_t)] = (char)((block_id >> (i * sizeof(block_id_t))) & 0xFF);
                    }
        ...
        else
        {
            ...
            for (usize idx = new_block_num; idx < old_block_num; ++idx)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < sizeof(block_id_t); i++)
                    {
                        block_id |= ((unsigned long long)indirect_block[i + block_idx * sizeof(block_id_t)]
                                     << (i * sizeof(block_id_t)));
                    }
                    ...
                    for (int i = 0; i < sizeof(block_id_t); i++)
                    {
                        indirect_block[i + block_idx * sizeof(block_id_t)] = (char)(0x00);
                    }
                    //                    UNIMPLEMENTED();
                }
            }
                ...
                new_block_num <= inlined_blocks_num && true)
            {
                ...
                is_indirect_block_in_memory = false;
            }
        }
...

        {
            ...
            while (write_sz < content.size())
            {
                ...
                block_id_t block_id = 0; // add by myself
                ...
                else
                {
                    ...
                    if (!is_indirect_block_in_memory)
                    {
                        if (block_manager_->read_block(inode_p->get_indirect_block_id(), indirect_block.data()).is_err())
                        {
                            return ChfsNullResult(ErrorType::INVALID);
                        }
                        is_indirect_block_in_memory = true;
                    ...
                    for (int i = 0; i < sizeof(block_id_t); i++)
                    {
                        auto indirect_block_idx = block_idx - inlined_blocks_num;
                        //                        printf("Indirect_block[%lu]:%d\n", i + indirect_block_idx * sizeof(block_id_t),
                        //                               (int) indirect_block[i + indirect_block_idx * sizeof(block_id_t)]);
                        block_id |= ((unsigned long long)indirect_block[i + indirect_block_idx * sizeof(block_id_t)]
                                     << (i * sizeof(block_id_t)));
                    }
                    //                    UNIMPLEMENTED();
                }
                ...
                // if (block_manager_->write_block(block_id, buffer.data()).is_err())
                // {
                //     return ChfsNullResult(ErrorType::INVALID);
                // }
                // auto op = std::make_shared<BlockOperation>(new BlockOperation(block_id, buffer));
                // bm->log_entry_->emplace_back(op);
                block_manager_->append_to_entry(block_id,buffer);
        ...
        // finally, update the inode
        {
            ...
            auto write_res =this->block_manager_->append_to_entry(inode_res.unwrap(),inode);
    ...
    auto FileOperation::read_file(inode_id_t id) -> ChfsResult<std::vector<u8>>
    {
        ...
        while (read_sz < file_sz)
        {
            ...
            else
            {
                ...
                auto block_idx = read_sz / block_size - inode_p->get_direct_block_num();
                for (int i = 0; i < sizeof(block_id_t); i++)
                {
                    block_id |= ((unsigned long long)indirect_block[i + block_idx * sizeof(block_id_t)]
                                 << (i * sizeof(block_id_t)));
            ...
            if (block_manager_->read_block(block_id, buffer.data()).is_err())
            {
                return ChfsResult<std::vector<u8>>(error_code);
            }
