...
namespace chfs {
    ...
    BlockManager::BlockManager(const std::string &file, usize block_cnt, bool is_log_enabled)
        ...
        if (is_log_enabled) {
            ...
            this->log_blocks = 1024;
            CHFS_ASSERT(this->block_cnt > this->log_blocks, "No enough space for logger.");
            this->block_cnt -= this->log_blocks;
        } else {
            this->log_blocks = 0;
        }
    }
    ...
    -> ChfsNullResult {
        ...
        // TODO: Implement this function.
        // UNIMPLEMENTED();
        if (block_id >= block_cnt+log_blocks || block_id < 0) {
            return ChfsNullResult(ErrorType::INVALID_ARG);
        } else {
            memcpy(reinterpret_cast<char *>(block_data + block_id * block_sz), reinterpret_cast<const char *>(data),
                   block_sz);
        }
    ...
    -> ChfsNullResult {
        ...
        if (block_id >= block_cnt+log_blocks || block_id < 0) {
            ...
            memcpy(reinterpret_cast<char *>(block_data + block_id * block_sz + offset),
                   reinterpret_cast<const char *>(data), real_len);
        }
    ...
    auto BlockManager::read_block(block_id_t block_id, u8 *data) -> ChfsNullResult {
        ...
        if (block_id >= block_cnt+log_blocks || block_id < 0) {
            ...
            if(log_entry_!= nullptr){
                for(int i=log_entry_->size()-1;i>=0;i--){
                    if((*log_entry_)[i]->block_id_==block_id){
                        memcpy(reinterpret_cast<char *>(data), reinterpret_cast<const char *>((*log_entry_)[i]->new_block_state_.data()),
                               block_sz);
                        return KNullOk;
                    }
                }
            }
            memcpy(reinterpret_cast<char *>(data), reinterpret_cast<const char *>(block_data + block_id * block_sz),
                   block_sz);
        }
    ...
    auto BlockManager::sync(block_id_t block_id) -> ChfsNullResult {
        if (block_id >= this->block_cnt +log_blocks) {
            return ChfsNullResult(ErrorType::INVALID_ARG);
        }
    ...
    auto BlockManager::flush() -> ChfsNullResult {
        auto res = msync(this->block_data, this->block_sz * (this->block_cnt+log_blocks), MS_SYNC | MS_INVALIDATE);
        if (res != 0)
            return ChfsNullResult(ErrorType::INVALID);
        return KNullOk;
    }
