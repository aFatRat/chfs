...
namespace chfs
{
        ...
        -> ChfsNullResult
    {
        ...
        {   
            ...
            buffer.insert(buffer.end(),new_dir.begin(),new_dir.end());
            this->write_file(parent,buffer);

            // auto res= this->inode_manager_->read_inode(parent, inode);
            // block_id_t parent_block_id=res.unwrap();
            // Inode *inode_p = reinterpret_cast<Inode *>(inode.data());
            // inode_p->set_size(new_dir.size());
            // this->block_manager_->write_block(parent_block_id,inode.data());

            // block_id_t parent_block_id = this->inode_manager_->get(parent).unwrap();
            // std::copy(new_dir.begin(), new_dir.end(), buffer.begin());
            // this->block_manager_->write_block(parent_block_id, buffer.data());
